...
def cmd_extract_text(args, project_dir: Path):
    ...
    # V4-skel.1 V4-2b 结构占位喊话: enumerate_inventory frontmatter 4 字段全 None
    print(
        f"[V4-2b 占位] assets_inventory.json 含 frontmatter 字段 "
        f"(review_status/valid_until/issuer/applicable_scope) 全 None, "
        f"值 V4-2b 实现层填",
        file=sys.stderr,
    )
    print()
    ...
    print('       "asset_query": {"type": "...", "name": "...",')
    print('                       "inventory_match": {"filename": "...", ...},')
    print('                       "rationale": "..." }}      ← V4-skel.2 V4-2b 结构占位')
    print("    ]")
    print("  }")
    print()
    print("  asset_query.rationale (V4-2b 结构占位): AI 选材理由文本。")
    print('  未填或值为 "__PENDING_AI__" 时 b_mode_fill 溯源行会喊 [V4-2b 占位]。')
    print('  详略约定 V4-2b 实现层填 (docs/v4_backlog.md §1 V4-2b 观察点 B)。')
    print()
    print("配套 sidecar (V4-skel.2 V4-2b 结构占位, AI 写, 跟 intermediate.json 同 part_dir):")
    print(f"  selection_rationale.json (保存到: {out_dir / 'selection_rationale.json'})")
    print("    {")
    print('      "candidates_total": <N>,')
    print('      "excluded_filenames": ["..."],')
    print('      "excluded_reasons": ["..."],')
    print('      "selected_filename": "...",')
    print('      "reasoning_text": "__PENDING_AI__"   ← V4-2b 实现层填')
    print("    }")
    print('  reasoning_text 跟 asset_query.rationale 同源 (前者详细版, 后者 inline)。')
    print()
