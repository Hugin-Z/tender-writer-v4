...
def main() -> int:
    ...
    # ─── case 1: enumerate_inventory 对 tempdir 真扫返候选 + 字段 + frontmatter 占位 ───
    # V4-skel.1: V4-2a 原 case 验"frontmatter 字段不在 (V4-2a 边界)"。V4-2b 结构占位
    # 起把字段加进 schema 但值留 None — 边界从"字段不在"扩到"字段在值 None,值实现层填"。
    # 测试同步: required_keys 加 4 个 V4-2b 占位字段, no_frontmatter 改为
    # no_frontmatter_value (验值是 None 即可,不验字段不在)。
    cases += 1
    with tempfile.TemporaryDirectory() as td:
        ...
        ok_count = len(inv) == 3
        # 字段验证: V4-2a 6 字段 + V4-skel.1 V4-2b 占位 4 字段 = 10 字段
        v4_2a_keys = {"category", "company_id", "filename", "year", "path", "kind"}
        v4_2b_placeholder_keys = {
            "review_status", "valid_until", "issuer", "applicable_scope",
        }
        required_keys = v4_2a_keys | v4_2b_placeholder_keys
        ok_keys = all(set(e.keys()) == required_keys for e in inv)
        # V4-skel.1 边界: V4-2b 占位字段值必须全 None (V4-2b 实现层才填)
        ok_no_frontmatter_value = all(
            all(e.get(k) is None for k in v4_2b_placeholder_keys) for e in inv
        )
        ...
        case1_ok = (ok_count and ok_keys and ok_no_frontmatter_value
                    and ok_year and ok_kind)
        icon = "PASS" if case1_ok else "FAIL"
        print(f"  [{icon}] case_1_enumerate_inventory_tempdir_scan")
        if not case1_ok:
            print(f"           count ok={ok_count} (got {len(inv)}), "
                  f"keys ok={ok_keys}, "
                  f"no_frontmatter_value ok={ok_no_frontmatter_value}, "
