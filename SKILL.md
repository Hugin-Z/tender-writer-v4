...
parse_tender.py 产出的 `extracted` 字段是空骨架(10 个字符串字段为 "" + qualifications 为 [] + substantial_response_marks 已由脚本抽出)。AI 必须读 `raw_lines_for_ai` 或 `tender_raw.txt` 原文,直接判定并填入以下 **11** 个字段:
...
**产物**:`output/scoring_matrix.csv`(10 列,用户必须 review 并确认)

**完成标志**:10 列填齐(第 6 列保留 placeholder),用户确认后推进到阶段 3。**禁止**跳过本阶段直接写提纲。
...
- **C-attachment**:产出 `attachments.yaml` 附件清单。附件作为独立提交件,不并入主响应文件文本流;V45 合并器会拷贝已存在的附件,并把缺失附件列入 `operations_checklist.md` / `pending_manual_work.md` 供投标前人工放置。
...
脚本内部按 sub_mode 分支;C-reference 的 `c_mode_fill` 仅做存在性校验不产 docx,C-attachment 的 `c_mode_fill` 校验并规范化附件清单。此一致性是刻意保留,便于用户对所有 C 模式 Part 用同一组命令处理。
...
- C-attachment: intermediate.json + attachments.yaml
...
- **inline_template**:招标文件给模板 + 变量占位(如基本情况表、信誉声明) → **产占位段供使用者手填,同 `self_drafted`**(自动调 c_mode_fill 不做,跨模块自动化无真实需求驱动,低频人工填更简单)
...
**V4-skel V4-7 结构层(2026-06)**:V45 整标合并器(`v45_merge.py`,涉 A/B/C 全模式 Part 合并,本备注就近写在此处,非仅 B 模式)在 `composer.append` 完成后、`composer.save` 前接入 `_post_merge_normalize` hook(当前 noop),并产 `final_tender_package/merge_normalize.log` 记录四维协调状态(`font_normalize` / `table_width_unify` / `section_break` / `page_number_reset` 当前全为 `"noop"` 字符串)。实线 cross-doc 协调(跨 Part 字体一致 / 表格列宽统一 / section 收敛 / 页码 reset)留 V4-7 实现层,待真实多 Part 合并产物的视觉冲突驱动;其深度与 V4-1a 表格/字体保真同类,需真实样本 + 人工目检兜底。

**V4-7a 浅做(2026-06)**:`_post_merge_normalize` 从全 noop 扩为接 master 规范一次 —— 调 `apply_default_styles(final_doc)` 解 docxcompose「后到优先」导致 anchor 自带 Heading 蓝色(`4F81BD`)注入并覆盖 master 黑色的问题(apply 后压回 `000000`);整篇 doc 内 `<w:tbl>` cell run 强 set `sz/szCs=24`(复用 V4-1a.12 `_apply_table_cell_size_to_runs`,scope 从单 part 扩到整篇),落回 V4-1a 表格小一号 12pt 约定。`merge_normalize.log` 五维诚实摊开:`font_normalize` 与 `table_cell_size` 两维 done、`table_width_unify` / `section_break` / `page_number_reset` 三维仍 noop。**V4-7b**(run-level normalize 整篇 + section 收敛 + 页码 reset)留实现层,待真实多 Part 合并产物的视觉冲突驱动,深度与 V4-1a 同类。
