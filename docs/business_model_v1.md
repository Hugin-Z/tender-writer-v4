...
### §8 #N20 · sub_mode 判据 + C-attachment 状态

**字面源**:`SKILL.md` 阶段 4-C "分流依据" + "sub_mode 判定" + `scripts/brief_schema.py` 的 `SUB_MODE_VALUES`
...
| **C-attachment** | `intermediate.json` + `attachments.yaml` 附件清单 | 独立附件不并入主响应文件;V45 合并器拷贝已存在附件,缺失项进入操作清单 / 待人工清单 |
...
**C-attachment 状态**:V60 落地时尚无真实 C-attachment 用例,曾以挂档状态保留 schema 值。V4-4 已启封该分支,`c_mode_extract` / `c_mode_fill` / `v45_merge` / `export_deliverables` 均按附件清单真实处理;附件源文件缺失时显式标占位,不静默产空附件。
...
**V4-2a 链路打通(2026-05-29)**:阶段 4-B 起,AI 写 `intermediate.json` 时由 `b_mode_extract.cmd_extract_text` 产出两个 sidecar —— `scoring_matrix_excerpt.md`(本 Part 关联评分项的关键词 + 证据材料)与 `assets_inventory.json`(`CuratedLocalAssetsProvider.enumerate_inventory()` 扫库的候选清单,仅文件系统可见信息,不读 frontmatter)。AI 据三件源(`raw` / `excerpt` / `inventory`)在 `asset_query.inventory_match` 记录选中的真实候选;库内无匹配按 `manifest_schema` 占位规则标占位,不盲写。此前 AI 写 `asset_query` 不知库内有什么(实测:库空仍盲写),V4-2a 消除该盲写。选材为 AI 语义判断,脚本只产候选清单。`inventory_match` 字段定义见 [`docs/manifest_schema.md`](manifest_schema.md);rationale + frontmatter 元数据匹配留 V4-2b。

**V4-1a B asset 保真 + 公章红线(2026-05-29)**:B 模式 asset 注入 assembled.docx 由「仅提取段落文本」升级为「OXML element 拷贝 + 跨 part ref name-based rebind」,表格(含 tblGrid 列宽)、段落级样式(`pStyle` 按 `w:name` 映射到 master 标准样式)、run 级字体/加粗保真;自动编号(`numId`)注入时 strip 并 warning(已知降级,用户可手补编号字面)。图片保真留 V4-1b。**公章整个 tender-writer 不做(原则级红线,非待办)**:公章是签章权的体现,要么物理盖红章、要么走电子签章系统,两者都必须是用户/签章主体的真实法律动作;工具生成公章等同代为签章/伪造,同「`.reviewed` 闸门只能用户人工创建,AI 不代建」红线同源。投标时公章页由用户手工处理。扫描件保真待定。
