...
(原 V4-6 D 模式待决策, Hugin 2026-06 评估决策不做, 已挪至 §5 "已完成 / 已决策不做" 段。)
...
| 5 | **MCPExternalAssetsProvider**(对接企业内部统一材料平台,通过 MCP 协议接外部材料库) | `scripts/assets_provider.py:14-17` docstring 预留意图 + `docs/v3_planning.md:334` "V3 不做留 V4" + `SKILL.md:837` "未来工作 V4+";V4 全期没接、连本 backlog 都没登记 — **本次顺手登记,只备查不做,状态:docstring 预留意图,未实现未排期**(跟 V4-6 信用中国不同:前者企业内部材料平台聚合,后者公网信用查询;Hugin 关心"统一材料平台"需求时再捞) |
...
- **V4-6 · D 模式 / "线上查资质"类自动化**(Hugin 2026-06 决策不做):D 模式(信用中国类"无违法记录"查询截图)**不做**。理由:低频(一个标查一两次指定公司)、人工查+过验证码+截图两分钟、半自动化省的时间 < 搭浏览器自动化+处理验证码+截图存档的工程成本,性价比不高;这类合规材料人工查截图处理(同"证书图人工放原件"的人机分工)。**注意**:这跟资质证书是两码事(信用记录截图是独立合规材料,官网截图本就是其合法形态),不是被"工具不代为生成签章"红线覆盖的对象,纯粹是性价比决策。
